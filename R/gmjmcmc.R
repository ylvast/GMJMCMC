...
#'
#' @export gmjmcmc
gmjmcmc <- function (data, loglik.pi, loglik.alpha, transforms, P = 10, N.init = 100, N.final = 100, probs = NULL, params = NULL, sub = FALSE) {
  ...
  for (p in seq_len(P)) {
    ...
    if (p != 1) data.t <- precalc.features(data, S[[p]])
    else data.t <- data
