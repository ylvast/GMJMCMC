...
gen.modification <- function (features, marg.probs, trans.probs, trans.priors, transforms) {
  ...
  new_feature <- list(transforms[trans],features[[feat]])
  return(create.feature(new_feature, features[[feat]], transforms, trans.priors))
}

# Generate a projection feature
gen.projection <- function (features, marg.probs, trans.probs, max.width, max.size, trans.priors, transforms) {
  if (!is.null(max.size)) {
  ...
  return(create.feature(new_feature, features[feats], transforms, trans.priors, alphas))
}
...
gen.drop <- function (features, marg.probs, transforms, trans.priors) {
  feat <- sample.int(n = length(features), size = 1, prob = marg.probs+0.00001)
  # If no sum or prod in the expression, nothing to remove
  if (!(any(unlist(features[[feat]]$eq) == "Sum") | any(unlist(features[[feat]]$eq) == "Prod"))){
    return(NULL)
  }
  ...
  remove_index <- sample(c(1:length(filtered_elements)), 1)
  remove_out <- filtered_elements[switch]
...
gen.switch <- function (features, marg.probs, transforms, trans.priors) {
...
gen.feature <- function (features, marg.probs, data, loglik.alpha, probs, F.0.size, params, transforms, verbose = TRUE) {
  tries <- 0
  feat.ok <- F
  while (!feat.ok && tries < 50) {
    feat.type <- sample.int(n = 6, size = 1, prob = probs$gen)
    if (feat.type == 1) feat <- gen.multiplication(features, marg.probs,transforms, probs$trans_priors)
    if (feat.type == 2) feat <- gen.modification(features, marg.probs, probs$trans, probs$trans_priors, transforms, probs$trans_priors)
    if (feat.type == 3) feat <- gen.projection(features, marg.probs, probs$trans, params$L, params$max.proj.size, probs$trans_priors, transforms)
    if (feat.type == 4) feat <- gen.new(features, F.0.size)
    if (feat.type == 5) feat <- gen.removal(features, marg.probs, transforms, probs$trans_priors)
    if (feat.type == 6) feat <- gen.switch(features,marg.probs, transforms, probs$trans_priors)
    ...
    if (!(feat$depth > params$D || feat$width > params$L)) {
      ...
      if (!is.null(feat)) {
        ...
        if (params$check.col && !check.collinearity(feat, feats, transforms, F.0.size, data, params$col.check.mock.data))
          feat.ok <- T
...
check.collinearity <- function (proposal, features, transforms, F.0.size, data, mock) {
...
gen.covariates <- function (count) {
  ...
  for (i in c(1:length(feats))){
    ...
    covariates[[i]] <- list(eq=equation, depth=0, width=1, oc=0, alphas=NULL)
